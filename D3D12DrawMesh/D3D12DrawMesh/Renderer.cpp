...
void FRenderer::RenderScene(FDynamicRHI* RHI, FFrameResource* FrameRes)
{
	...
	for (auto i : FrameRes->MeshActorFrameReses)
	{
		...
		// root signature
		RHI->SetShaderInput(FPassType::SHADOW_PASS, i.MeshRes.get(), FrameRes);

		// set mesh
		RHI->SetMeshBuffer(i.Mesh.get());
	}
	...
	{
		// pso
		RHI->ChoosePipelineState(i.MeshRes->OutputPassPso.get());

		// root signature
		RHI->SetShaderInput(FPassType::BASE_PASS, i.MeshRes.get(), FrameRes);
